package net.luis.cave.util.enums;
...
public enum CaveItemTier implements IItemTier {
	...
	LIMONITE(3, 2031, 11.0f, 4.0f, 23, () -> {
		...
		return Ingredient.fromItems(CaveItems.LIMONITE_INGOT.get());
		
	}),
	
	JADE(2, 1561, 7.0f, 6.0f, 65, () -> {
		
		return Ingredient.fromItems(CaveItems.JADE_INGOT.get());
		
	}),
	
	BLAZING(3, 1247, 8.0f, 6.0f, 15, () -> {
		
		return Ingredient.fromItems(CaveItems.BLAZING_INGOT.get());
		
	}),
	
	RUBY(3, 2031, 7.0f, 7.0f, 17, () -> {
		
		return Ingredient.fromItems(CaveItems.RUBY.get());
		
	}),
	
	SAPHIRE(3, 1746, 10.0f, 6.0f, 28, () -> {
		
		return Ingredient.fromItems(CaveItems.SAPHIRE_INGOT.get());
		
	}),
	
	ROSITE(2, 1287, 6.5f, 7.0f, 24, () -> {
		
		return Ingredient.fromItems(CaveItems.ROSITE_INGOT.get());
		
	}),
	
	ROSE_QUARTZ(1, 517, 6.0f, 10.0f, 27, () -> {
		
		return Ingredient.fromItems(CaveItems.POLISHED_ROSE_QUARTZ.get());
		
	}),
	
	STEEL(4, 18473, 15.0f, 6.0f, 35, () -> {
		
		return Ingredient.fromItems(CaveItems.STEEL_INGOT.get());
		
	}),
	
	ENDERITE(4, 4635, 13.5f, 20.0f, 38, () -> {
		
		return Ingredient.fromItems(CaveItems.ENDERITE_INGOT.get());
